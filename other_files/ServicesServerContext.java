...
import org.cendra.commons.util.service.AbstractServicesServerContext;
import org.cendra.commons.util.service.ResponseEntity;
import org.cendra.commons.util.service.ServiceMetaData;
import org.cendra.commons.util.service.Verbs;
...
public class ServicesServerContext extends AbstractServicesServerContext {

	private DataSourceProperties dataSourceProperties;
	private DataSource dataSource;
	private DataSourceWrapper dataSourceWrapper;

	public ServicesServerContext() {
		super();
		try {
			// init("Postgresql");
			init("sqlserver");
	...
	protected void initServicesMetaData() {
		ServiceMetaData serviceMetaData = new ServiceMetaData();
		serviceMetaData.setVerb(Verbs.GET);
		serviceMetaData.setUrl("/ejercicios_contables");
		serviceMetaData.setResponseBodyIsList(true);

		servicesMetaData.put(serviceMetaData.getId(), serviceMetaData);
	}
	...
	protected Object execute(String verb, String url,
			Map<String, Object> requestParams, Object requestBody)
			throws Exception {

		String id = "[" + verb + "] " + url;

		if (id.equalsIgnoreCase("[GET] /ejercicios_contables")) {
			EjercicioContableBO bo = new EjercicioContableBO(
					new EjercicioContableDAO(dataSourceWrapper));

			if (requestParams.containsKey("ejercicioEndsWith")
					&& requestParams.get("ejercicioEndsWith") != null) {
				return bo.findAll(requestParams.get("ejercicioEndsWith")
						.toString());
			} else {
				return bo.findAll();
			}
		...
		throw new Exception("BO not found. " + id);
	}
